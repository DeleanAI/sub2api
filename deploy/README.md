...
The command prints, per storage location, how many rows were rotated, were already current, or failed (with row ids). Failed rows are ciphertext no configured key can open — typically data written by an instance that ran with a different (or auto-generated) key; reset those secrets through the admin UI and re-run. Rows holding **legacy plaintext** (no version prefix, not decryptable by any configured key — these fields historically allowed unencrypted values) are encrypted with the primary key instead of counting as failures, and are reported on a separate `note:` line; a rotation is not blocked by them. Legacy payment provider configs are converted to the current plaintext format in the same pass. `--old-key-file`/`--new-key-file` read a key from a file (`-` = stdin) and override the two variables for a single run — keys are deliberately not accepted as flag values, since argv is readable from `/proc/<pid>/cmdline` and lands in shell history.
