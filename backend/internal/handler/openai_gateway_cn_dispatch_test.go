...
func TestAllowOpenAICompatibleMessagesDispatch_CNProvidersExempt(t *testing.T) {
	...
	for _, platform := range []string{service.PlatformKimi, service.PlatformZhipu, service.PlatformDeepseek, service.PlatformQwen, service.PlatformGrok, service.PlatformMiniMax, service.PlatformOpenCodeGo} {
...
func TestAllowOpenAICompatibleMessagesDispatch_CompositeResolvedTargets(t *testing.T) {
	...
	for _, model := range []string{"grok-4.3", "kimi-k2-thinking", "glm-5.2", "deepseek-v3.2", "qwen3-max"} {
...
func TestResolveOpenAIMessagesDispatchMappedModel_CompositeCNTargetsSkipGroupMapping(t *testing.T) {
	...
	for _, model := range []string{"kimi-k2-thinking", "glm-5.2", "deepseek-v3.2", "qwen3-max", "grok-4.3"} {
		c, _ := gin.CreateTestContext(httptest.NewRecorder())
		c.Request = httptest.NewRequest("POST", "/v1/messages", nil)
