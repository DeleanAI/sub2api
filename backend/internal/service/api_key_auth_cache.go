...
type APIKeyAuthGroupSnapshot struct {
	...
	// ModelRateMultipliers 分组逐模型倍率：扣费热路径（CalculateCostUnified）与利润门 D 都从
	// 认证快照物化出的 Group 上读它，漏掉即静默失效（TestAuthGroupSnapshotRoundTripCoversEveryField 兜底）。
	ModelRateMultipliers            []GroupModelRateMultiplier `json:"model_rate_multipliers,omitempty"`
