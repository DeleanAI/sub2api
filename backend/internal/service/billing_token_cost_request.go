...
func (s *BillingService) CalculateTokenCostForRequest(req TokenCostRequest) (*CostBreakdown, error) {
	...
	// 兜底路径同样走统一入口：分组逐模型倍率只在 CalculateCostUnified 里施加，
	// 从这里直接调 CalculateCost 会让该因子在"无解析器/无分组定价"的请求上静默失效
	// （TestModelRateMultiplierInvariantAcrossGatewayPaths 钉住这一条）。
	// 统一入口在无 Resolver 时走的就是内置目录定价，与 CalculateCost 同源，
	// 区别只是会尊重 ServiceTier —— 与其余所有计费路径一致。
	return s.CalculateCostUnified(s.tokenCostInput(req, resolved))
}
