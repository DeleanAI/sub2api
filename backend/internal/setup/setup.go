...
func buildDatabaseConnectionDSNs(cfg *DatabaseConfig) (bootstrapDSN, targetDSN string) {
	conn := cfg.connection()
	return conn.DSNForDatabase(postgresBootstrapDatabase), conn.DSN()
}

// buildPostgresDSN 渲染连到指定库的 DSN。渲染规则仍然只有 config 包一份实现，
// 这里不自己拼串——否则 DATABASE_URL 带来的 ExtraParams（如 sslrootcert）与 libpq 引用规则会被悄悄丢掉。
func buildPostgresDSN(cfg *DatabaseConfig, dbName string) string {
	return cfg.connection().DSNForDatabase(dbName)
}
