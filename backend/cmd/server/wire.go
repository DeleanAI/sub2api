...
func provideCleanup(
	...
	return func() {
		...
		parallelSteps := []cleanupStep{
			// 先于 Redis 关闭注销自己，其他实例的存活计数才能立刻减一，而不是等 45 秒窗口。
			{"InstanceRegistry", func() error {
			...
			{"RequestPayloadAuditService", func() error {
				if requestPayloadAudit != nil {
					requestPayloadAudit.Stop()
				}
				return nil
			}},
