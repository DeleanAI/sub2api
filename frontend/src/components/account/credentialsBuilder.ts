...
export function isHeaderOverrideCapable(platform: string, type: string): boolean {
  if (
    platform === 'anthropic' ||
    platform === 'openai' ||
    // CN 供应商（kimi/zhipu/deepseek/qwen/minimax，名单只在 cnProviders 声明）
    // 加上复用同一表单的 OpenCode。
    isCnFormPlatform(platform)
  ) {
    return type === 'apikey'
  }
  if (platform === 'grok') {
...
/**
 * CnProviderPlatform 是 @/constants/cnProviders 里 canonical 平台集的别名。
 * 平台名单只在那一处声明——这里曾经各写一份（一份含 qwen、一份含 minimax），
 * 结果两边都漏了对方的平台。新增供应商只改 cnProviders.ts。
 */
export type CnProviderPlatform = CanonicalCnProviderPlatform
...
/** 委托给唯一的归一化入口，别在这里重复平台名单。 */
export function isCNProviderPlatform(platform: string): platform is CnProviderPlatform {
  return isCnProviderPlatform(platform)
}

/**
 * DeepSeek、Kimi、MiniMax 与 OpenCode 提供原生 Responses 端点。
 *
 * 归一化在**函数内部**做，调用方一律传原始 platform。此前 5 个调用点各写一遍
 * `cnSupportsNativeResponses(normalizeCnProviderPlatform(p) ?? '')`，而归一化只认
 * CN 供应商——opencode_go 归一化后是 null，空串会让它的 responses 档位整个消失
 * （实测：EditAccountModal 的 OpenCode Zen 用例提交时丢掉 responses 端点）。
 */
export function cnSupportsNativeResponses(platform: string | null | undefined): boolean {
  const raw = (platform ?? '').trim()
  if (!raw) return false
  const p = normalizeCnProviderPlatform(raw) ?? raw
  return p === 'deepseek' || p === 'kimi' || p === 'minimax' || p === 'opencode_go'
}
...
export const CN_BASE_URL_PRESETS: Record<CanonicalCnProviderPlatform | CnProviderPlatform, CnBaseUrlPreset[]> = {
  kimi: [
...
export function defaultCNBaseUrl(
  ...
  // 归一化认 CN 供应商（含别名）；不认的平台（opencode_go）回退原值，
  // 类型用 CnFormPlatform 以容纳它。
  const normalizedPlatform = (normalizeCnProviderPlatform(platform) ?? platform) as CnFormPlatform
  ...
  // 用归一化后的平台名：qwen 有 dashscope 等别名，未归一化会漏判；归一化不认的
  // 平台（minimax / opencode_go）回退原值继续匹配下面的分支。
  switch (normalizedPlatform) {
    case 'kimi':
...
export function defaultCNAdaptiveBaseUrls(
  ...
  return {
    ...
    // 走 cnSupportsNativeResponses 而不是写死 deepseek：上游已把 Kimi / MiniMax /
    // OpenCode 也纳入，而 qwen 的 preset 里没有 responses 协议，这个判据天然排除它。
...
export function cnQuotaCellVisible(platform: string, accountMode: string): boolean {
  // OpenCode：Go 订阅有额度窗口，Zen 是按量付费没有。
  if (platform === 'opencode_go') return accountMode !== 'zen'
  return (
    // Coding Plan（kimi / zhipu / minimax）有滚动用量窗口。
    (isCnCodingPlanPlatform(platform) && accountMode === 'coding') ||
    // Qwen Token Plan 是一次性额度，始终展示状态（耗尽标记也在这个单元格里）。
