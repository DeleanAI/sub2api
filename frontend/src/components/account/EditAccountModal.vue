...
const editApiKeyPlaceholder = computed(() => {
  ...
  switch (normalizeCnProviderPlatform(props.account.platform) ?? props.account.platform) {
    case 'openai':
      return 'sk-proj-...'
    case 'gemini':
      return 'AIza...'
...
// ── 国产供应商（Kimi / Zhipu / DeepSeek / Qwen）account_mode / api_protocol 编辑 ──
// account_mode 决定额度/余额监控路径，api_protocol 决定转发端点与格式；
// 二者均可修正（早期创建的账号可能存错默认值），切换时重置 base_url 预置。
const isCNApiKeyAccount = computed(
  () =>
    props.account?.type === 'apikey' &&
    // 平台判据取两套注册表的并集：@/constants/cnProviders 认 kimi/zhipu/deepseek/qwen，
    // credentialsBuilder 的 isCNProviderPlatform 认 kimi/zhipu/deepseek/minimax，
    // 少一边就会漏掉 qwen 或 minimax 账号的 account_mode / api_protocol 编辑入口。
...
const cnPresetPlatform = computed<CanonicalCnProviderPlatform | CnProviderPlatform>(() => {
  const platform = props.account?.platform
  const canonical = normalizeCnProviderPlatform(platform)
...
const adaptivePresetPlatform = computed<CanonicalCnProviderPlatform | CnProviderPlatform | 'opencode_go'>(() => {
...
const cnAccountModeOptions = computed<Array<{ value: CnAccountMode; labelKey: 'payg' | 'coding' | 'tokenPlan' }>>(
  () => {
    const platform = normalizeCnProviderPlatform(props.account?.platform)
    if (platform === 'qwen') {
      return [
        { value: 'payg', labelKey: 'payg' },
        { value: 'token_plan', labelKey: 'tokenPlan' }
      ]
    }
...
const cnProtocolOptions = computed<Array<{ value: CnApiProtocol; labelKey: string }>>(() => {
  ...
  // cnSupportsNativeResponses 内部做归一化，这里传原始 platform 即可。
...
watch(editAccountMode, (mode, previousMode) => {
  if (!isCNApiKeyAccount.value || syncingForm.value) return
  // OpenCode 的模式（zen/go）由 editOpenCodeAccountMode 单独管理，不走 CN 回退。
  if (props.account?.platform === 'opencode_go') return
  // 防御性回退：DeepSeek 无 coding 套餐；Qwen 不使用 coding，旧别名的
  // 非按量模式在编辑 UI 中统一视作一次性 Token Plan。
  const platform = normalizeCnProviderPlatform(props.account!.platform)
  const effectiveMode = platform === 'qwen'
    ? (mode === 'payg' ? 'payg' : 'token_plan')
    : platform === 'deepseek' && mode === 'coding'
      ? 'payg'
      : mode === 'token_plan'
        ? 'payg'
        : mode
...
const readOpenAIEndpointCapabilities = (credentials?: Record<string, unknown>): OpenAIEndpointCapability[] => {
  ...
  if (Array.isArray(raw)) {
    return normalizeOpenAIEndpointCapabilities(raw.filter(isOpenAIEndpointCapability))
  }
  ...
  return [...DEFAULT_OPENAI_ENDPOINT_CAPABILITIES]
}
...
const applyOpenAIEndpointCapabilities = (credentials: Record<string, unknown>) => {
  ...
  if (isDefaultOpenAIEndpointCapabilitySet(capabilities)) {
...
const defaultBaseUrl = computed(() => {
  ...
  // CN 供应商与 OpenCode：按当前模式/协议回落到官方预设（清空输入框提交时使用），
  // 不能落到 anthropic 默认值（会被当 CC base 拼出错误端点）。
  // 判据取两套注册表的并集：isCnProviderPlatform 认 kimi/zhipu/deepseek/qwen。
  if (isCnFormPlatform(platform)) {
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    if (
      ...
      } else if (isQwenPlatform(newAccount.platform)) {
        // Qwen 的默认模式是 Token Plan（一次性额度），不是 payg。
        editAccountMode.value = credentials.account_mode === 'payg' ? 'payg' : 'token_plan'
      } else {
