import { normalizeCnProviderPlatform } from '@/constants/cnProviders'
...
const BADGE: Record<Platform, string> = {
  ...
  qwen: 'bg-sky-500/10 text-sky-700 border-sky-500/30 dark:text-sky-300',
...
const BADGE_LIGHT: Record<Platform, string> = {
  ...
  qwen: 'bg-sky-500/10 text-sky-700 dark:bg-sky-500/10 dark:text-sky-300',
...
const BORDER_STRONG: Record<Platform, string> = {
  ...
  qwen: 'border-sky-500/35 dark:border-sky-500/30',
...
const ICON: Record<Platform, string> = {
  ...
  qwen: 'text-sky-500 dark:text-sky-300',
...
const BUTTON: Record<Platform, string> = {
  ...
  qwen: 'bg-sky-600 text-white hover:bg-sky-700 active:bg-sky-800 dark:bg-sky-600/80 dark:hover:bg-sky-600',
...
function resolvePlatform(p: string): Platform | null {
  const normalizedCnProvider = normalizeCnProviderPlatform(p)
  const candidate = normalizedCnProvider ?? p
